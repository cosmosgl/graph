...
**New methods:**
- `findPointsInRect(rect)` — returns `number[]` of point indices in a rectangular area (synchronous; must be called when graph is ready)
- `findPointsInPolygon(polygonPath)` — returns `number[]` of point indices in a polygon area (synchronous; must be called when graph is ready)
