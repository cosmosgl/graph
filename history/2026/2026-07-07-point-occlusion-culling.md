...
On this branch the culling applies in **2D mode only**: in 3D, points already write
real perspective depth and index order is unrelated to camera order, so the
reversed-index "front-to-back" pass would be meaningless — 3D rendering keeps its
...
|---|---|
| `true` (default) | On whenever safe — active in 2D mode while `pointOpacity` is `1` and `highlightedPointIndices` is not set; otherwise the standard path is used (highlighting's layered greyed/highlighted rendering relies on paint order) |
| `false` | Off — always use the standard single-pass rendering (escape hatch for driver/GPU trouble) |

This is the simplified boolean shape agreed on PR #238 — the original tri-state
(`undefined` = auto / `true` = force / `false` = off) was dropped because forcing with
`pointOpacity < 1` can't produce any core-pass fragments and is pure overhead; auto
already activates everywhere the optimization can help.
...
- `Points.draw()` picks the path per frame; pass-B parameters flip between
  `FRINGE_PASS_PARAMETERS` and `DEFAULT_DRAW_PARAMETERS_2D` only on an actual mode
  change (same pattern as `Lines.updateLinkBlending`). In 3D both sides of that check
  stay `false`, so the 3D pipeline parameters are never touched.
- `drawCoreCommand` is created only in 2D mode and is destroyed/recreated in the same
  `programsSpaceDimensions` mode-flip rebuild block in `initPrograms()` as
  `drawCommand`, so a runtime `spaceDimensions` switch gets correctly compiled Models.
...
- Hover/focus rings draw after both passes without depth testing in 2D and stay on
...
overdraw) with `showFPSMonitor: true`, a button toggling the culling on/off, and a
toggle that makes 30% of points translucent to demonstrate mixed-alpha correctness.
...
- Memory: +4 bytes/point for the reversed index buffer (2D mode only); each
  draw-uniform UBO grows by one f32.
