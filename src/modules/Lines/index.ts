...
export class Lines extends CoreModule {
  ...
  public updateLinkStatus (): void {
    ...
    const linksNumber = data.linksNumber ?? 0
    // No links yet — keep a placeholder so the GPU binding is always satisfied
    if (!linksNumber) {
      this.resetLinkStatusToPlaceholder()
      return
    }

    const { highlightedLinkIndices } = config
    // Highlighting cleared — fall back to placeholder; shader won't sample it
    // because isLinkHighlightingActive uniform is set to 0 in this case
  ...
  // luma.gl silently skips the draw call if any texture declared in the shader is not bound.
  // Since linkStatus is always declared, we keep a 1×1 dummy bound when highlighting is off.
  // The shader won't sample it — the isLinkHighlightingActive uniform guards that branch.
  private resetLinkStatusToPlaceholder (): void {
    const { device } = this
    if (this.linkStatusTexture && !this.linkStatusTexture.destroyed) {
      this.linkStatusTexture.destroy()
    }
