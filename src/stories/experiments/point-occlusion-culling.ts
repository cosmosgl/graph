 ...
 * effect of depth-based occlusion culling (applies in 2D mode). The
 * "translucent points" toggle gives a share of the points a semi-transparent
 * color to demonstrate that mixed-alpha scenes still blend correctly in
 * stacking order.
 */
export const pointOcclusionCulling = (): { graph: Graph; div: HTMLDivElement; destroy?: () => void } => {
  ...
  // Toggle the pointOcclusionCulling boolean (default: true = on when safe)
  let occlusionCullingEnabled = true
  const applyOcclusionState = (): void => {
    graph.setConfigPartial({ pointOcclusionCulling: occlusionCullingEnabled })
    occlusionButton.textContent = `Occlusion culling: ${occlusionCullingEnabled ? 'On' : 'Off'}`
  }
  applyOcclusionState()
