...
export const collision = (): { graph: Graph; div: HTMLDivElement } => {
  ...
  // The spatial-hash collision force can handle many points efficiently.
  const numPoints = 500
  ...
  for (let i = 0; i < numPoints; i++) {
    // Start with a Gaussian-like distribution in the center
    ...
    // Varying sizes (bigger points near the center)
