...
The returned array contains the processed point sizes used for rendering, including any default values that were applied during processing. While point sizes transition, it holds the target sizes; `getPointRadiusByIndex` moves with the animation.
...
The returned array contains the processed link widths used for rendering, including any default values that were applied during processing. While link widths transition, it holds the target widths; `getLinkWidthByIndex` moves with the animation.

### <a name="get_link_width_by_index" href="#get_link_width_by_index">#</a> graph.<b>getLinkWidthByIndex</b>(<i>index</i>)

This method retrieves the width of the link at <i>index</i> as it is drawn. While link widths transition after `setLinkWidths`, the value moves with the animation; otherwise it is the width `getLinkWidths` holds.

* **`index`** (Number): The index of the link.

**Returns:** The link's width, or `undefined` for an index outside the links.
...
This method retrieves the radius of a point in the graph identified by its <i>index</i>, as it is drawn: while point sizes transition after `setPointSizes`, the value moves with the animation.
