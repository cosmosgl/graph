...
export const collision = (): { graph: Graph; div: HTMLDivElement } => {
  ...
  const pointCluster = new Array<number>(numPoints).fill(0)
  ...
  for (let i = 0; i < numPoints; i++) {
    ...
    const [cx, cy] = clusterCenters[cluster] as [number, number]
  ...
  for (let i = 0; i < numPoints; i++) byCluster[pointCluster[i] as number]!.push(i)
  ...
  for (const members of byCluster) {
    for (const point of members) {
      ...
      for (let k = 0; k < connections; k++) {
        const other = members[Math.floor(Math.random() * members.length)] as number
        addLink(point, other)
      }
    }
  }
  ...
  for (let i = 0; i < numPoints; i++) {
    const hubness = (degree[i] as number) / maxDegree
    pointSizes[i] = 4 + hubness * 24 + Math.random() * 5

    const rgba = getRgbaColor(clusterColorScale(pointCluster[i] as number))
  ...
  for (let i = 0; i < linkCount; i++) {
    ...
    const rgba = getRgbaColor(clusterColorScale(pointCluster[source] as number))
