...
**3D mode limitations:** area selection (`findPointsInRect` / `findPointsInPolygon`), collision, clusters, and right-click repulsion are disabled in 3D mode — the corresponding methods warn and do nothing. Use the `3D` counterparts of the zoom and sampling APIs: [`fitView`](#fit_view) works in both modes, and [`getSampledPoints3D`](#get_sampled_points_3d) / [`spaceToScreenPosition3D`](#space_to_screen_position_3d) replace their 2D versions. The camera is configured via the `cameraFov`, `cameraNear`, `cameraFar` and `cameraInitialPosition` [configuration properties](../?path=/docs/configuration--docs).
