...
### <a name="set_point_positions_3d" href="#set_point_positions_3d">#</a> graph.<b>setPointPositions3D</b>(<i>pointPositions</i>)
...
**3D mode limitations:** area selection (`findPointsInRect` / `findPointsInPolygon`) and right-click repulsion are disabled in 3D mode — the corresponding methods warn and do nothing. Collision and cluster forces run in 3D; `setClusterPositions3D` and `getClusterPositions3D` are the 3D counterparts of the cluster-position APIs. Use the `3D` counterparts of the zoom and sampling APIs: [`fitView`](#fit_view) works in both modes, and [`getSampledPoints3D`](#get_sampled_points_3d) / [`spaceToScreenPosition3D`](#space_to_screen_position_3d) replace their 2D versions. The camera is configured via the `cameraFov`, `cameraNear`, `cameraFar` and `cameraInitialPosition` [configuration properties](../?path=/docs/configuration--docs).
