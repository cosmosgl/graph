...
**Simulation in 3D:** the force simulation runs in 3D with the same `simulation*` configuration options — gravity, centering, link springs, friction, and decay all work on 3D coordinates. Many-body repulsion in 3D uses an exact O(n²) GPU pass (instead of the 2D quadtree approximation), which is practical up to roughly 10–20k points.
