...
Calling `setPointPositions` also switches the graph back to 2D mode if it was in [3D mode](#set_point_positions_3d).
...
This method sets 3D positions for the graph points and switches the instance into **3D rendering mode**: a perspective orbit camera replaces the 2D pan/zoom (drag rotates around the data, mouse wheel / pinch dollies in and out, Space + drag pans), and points and links are rendered with perspective and depth.

* **`pointPositions`** (Float32Array): A Float32Array representing the x, y and z coordinates of points in the format `[x1, y1, z1, x2, y2, z2, ..., xN, yN, zN]`. Each triplet represents the coordinates of a single point.
* **`dontRescale`** (Boolean, optional): When `true`, skips the one-time rescaling of the provided positions into the `[0, spaceSize]` cube (mirrors the [`setPointPositions`](#set_point_positions) parameter).

On the next `render()`, points animate from their previous positions to the new ones using `transitionDuration` and `transitionEasing` (z animates from `0` when switching from 2D mode). Calling [`setPointPositions`](#set_point_positions) switches back to 2D mode.

**Simulation in 3D:** the force simulation runs in 3D with the same `simulation*` configuration options — gravity, centering, link springs, friction, and decay all work on 3D coordinates. Many-body repulsion in 3D uses an octree approximation above ~4k points (an exact O(n²) pairwise pass below), analogous to the 2D quadtree; `simulationRepulsionTheta` is not used in 3D.

**Dragging in 3D:** with `enableDrag: true`, points can be dragged in 3D — the dragged point moves in the camera-facing plane through its position at drag start. A drag that starts on a hovered point moves the point; everywhere else it orbits the camera.
...
**Example:**
```javascript
graph.setPointPositions3D(new Float32Array([
...
### <a name="set_camera_state" href="#set_camera_state">#</a> graph.<b>setCameraState</b>(<i>state</i>)
