...
Each shape value in the array specifies the shape of a point using the same order as the points in the graph data. Invalid shape values (outside the range 0-8) will default to the `pointDefaultShape` config value (Circle by default).
