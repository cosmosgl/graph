...
* **`div`** (HTMLDivElement): The HTML div element that will contain the graph canvas. The canvas will be automatically appended to this div.
* **`config`** (Object, optional): Configuration object for customizing graph behavior, appearance, and interaction settings. See [Configuration](../?path=/docs/configuration--docs) for available options.
* **`devicePromise`** (Promise&lt;Device&gt;, optional): Optional promise that resolves to a Luma.gl Device instance. If provided, this device will be used instead of creating a new one internally.

  **Requirements:**
  - The device must have an HTMLCanvasElement canvas context (not OffscreenCanvas).
  - Compute-only devices are not supported.
  - The canvas from the device will be automatically appended to the provided div if it's not already present in the DOM.
  
  **Important:** When providing an external device, the Graph's `config.pixelRatio` setting will override the device's `useDevicePixels` value to ensure consistency. If you need a specific pixel ratio, set it explicitly in the config (e.g., `{ pixelRatio: 1 }`). If not specified, it defaults to `window.devicePixelRatio || 2`.
  ...
  **Example:**
  ...
  // Use the custom device when creating the graph
