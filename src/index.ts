...
export class Graph {
  ...
  public constructor (
    ...
    const setupPromise = this.deviceInitPromise.then(device => {
      ...
      this.store.setOutlinedPointRingColor(this.config.outlinedPointRingColor ?? defaultConfigValues.outlinedPointRingColor)
  ...
  */
   ...
   * @returns Promise resolving to an array of point indices inside the rectangle.
   */
  public async findPointsInRect (rect: [[number, number], [number, number]]): Promise<number[]> {
    if (this._isDestroyed) return []
    await this.ready
   ...
   * The coordinates should be from 0 to the width/height of the canvas.
   * @returns Promise resolving to an array of point indices inside the polygon.
   */
  public async findPointsInPolygon (polygonPath: [number, number][]): Promise<number[]> {
    if (this._isDestroyed) return []
    await this.ready
    if (!this.device || !this.points) return []
    ...
    this.points.findPointsInPolygon()
    return extractIndicesFromPixels(readPixels(this.device, this.points.searchFbo as Framebuffer))
  }
